...
- DNS Service
  ...
  - Key files: `Dns/Core/AstrolabedService.cs`, `Dns/Core/DnsParser.cs`, `Dns/RuleEngine/DnsCache.cs`.
...
- DHCP Service
  ...
  - Key files: `Dhcp/CidrPoolAllocator.cs`, `Dhcp/*` (packet codecs and helpers under `src/Astrolabed/Dhcp`).
...
- NTP Service
  ...
  - Key files: `Ntp/*` under `src/Astrolabed/Ntp` (see repo tree for exact filenames).
...
## Data flow (detailed)
DNS request flow:
1. UDP packet arrives and is passed to `AstrolabedService.ProcessAsync`.
