    ...
    >
      <slot name="tooltip" :active="activeSegment">
        <template v-if="activeSegment">
          {{ activeSegment.barLabel }} ({{ activeSegment.seriesLabel }}):
          {{ activeSegment.value }} ({{ activeSegment.percentage.toFixed(1) }}%)
        </template>
      </slot>
    </div>
  </div>
...
const maxBarTotal = computed<number>(() => {
  const max = Math.max(
    ...props.modelValue.map((bar) => Object.values(bar.values).reduce((sum, val) => sum + val, 0)),
    1
  )
