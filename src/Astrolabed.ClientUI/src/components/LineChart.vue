    ...
    <div class="relative w-full aspect-[21/9] min-h-[280px]">
      ...
      >
        ...
        <g class="stroke-slate-700/40" stroke-dasharray="4 4" stroke-width="1">
          ...
          <line :x1="paddingLeft" y1="260" :x2="width - paddingRight" y2="260" stroke-dasharray="0" class="stroke-slate-700" />
        </g>
      ...
      >
        ...
        <span class="text-indigo-400 font-bold text-sm">${{ formatValue(hoveredPoint.value) }}</span>
      </div>
    </div>
  </div>
</template>
...
const stepWidth = computed(() => {
  return props.modelValue.length > 1 ? chartWidth.value / (props.modelValue.length - 1) : chartWidth.value;
});
