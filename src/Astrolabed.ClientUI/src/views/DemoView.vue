<template>
  <main class="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 sm:p-8">
    <div class="w-full max-w-4xl space-y-6">
      <!-- Top Section: Interactive Demo Controls -->
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-wrap gap-4 items-center justify-between">
        <div>
          <h1 class="text-xl font-bold text-white">Interactive Chart Visualizations</h1>
          <p class="text-xs text-slate-400 mt-0.5">Test reactivity, custom slots, and click events across Pie and Stacked Bar components</p>
        </div>
          ...
          <template #tooltip="{ activeSlice }">
            <div v-if="activeSlice" class="flex flex-col gap-0.5 p-0.5">
              <span class="font-bold text-white flex items-center gap-1.5">
                <span class="w-2 h-2 rounded-full" :style="{ backgroundColor: activeSlice.color }"></span>
                {{ activeSlice.label }}
              </span>
              <span class="text-slate-300 text-[11px]">
                Value: <span class="text-emerald-400 font-medium">${{ activeSlice.value.toLocaleString() }}</span>
              </span>
        ...
        >
          <template #tooltip="{ active }">
            <div v-if="active" class="flex flex-col gap-0.5 p-0.5">
              ...
              <span class="text-slate-300 text-[11px]">
                Value: <span class="text-emerald-400 font-medium">${{ active.value.toLocaleString() }}</span>
              </span>
              <span class="text-slate-400 text-[10px] italic">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-xs">
          ...
          <div v-if="lastClickedSlice" class="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="w-3 h-3 rounded-full" :style="{ backgroundColor: lastClickedSlice.color }"></span>
              <span class="font-bold text-white">{{ lastClickedSlice.label }}</span>
            </div>
            <div class="flex gap-4 text-slate-400">
              <span>Value: <strong class="text-slate-200">{{ lastClickedSlice.value }}</strong></span>
              <span>Share: <strong class="text-slate-200">{{ lastClickedSlice.percentage.toFixed(1) }}%</strong></span>
            </div>
          </div>
        </div>

        <!-- Stacked Bar Click Log -->
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-xs">
          <div class="flex items-center justify-between mb-2">
          ...
          <div v-if="lastClickedSegment" class="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="w-3 h-3 rounded-full" :style="{ backgroundColor: lastClickedSegment.color }"></span>
              <span class="font-bold text-white">{{ lastClickedSegment.barLabel }} ({{ lastClickedSegment.seriesLabel }})</span>
            </div>
            <div class="flex gap-4 text-slate-400">
              <span>Value: <strong class="text-slate-200">{{ lastClickedSegment.value }}</strong></span>
              <span>Share: <strong class="text-slate-200">{{ lastClickedSegment.percentage.toFixed(1) }}%</strong></span>
            </div>
          </div>
        </div>
      </div>
